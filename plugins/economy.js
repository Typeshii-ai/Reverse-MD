...
smd({
    ...
    async({reply, chat ,isGroup, sender,error}) => {
      try{
        ...
        const daily  = await eco.daily(sender, "QUEEN_ALYA", 500); //give 500 for daily, can be changed
        if (daily.cd) {
...
smd({
    ...
    async(message) => {
      try{
       ...
       const balance  = await eco.balance(users, "REVERSE KING")
       await eco.deduct(users, "REVERSE KING", balance.wallet);
...
async(message,match) => {
  try{
   ...
   const balance  = await eco.balance(user, "REVERSE KING")
   switch (value) {
       case '1000':
       case '1':
       if (k > balance.wallet ) return message.reply(`*_You need to pay 🪙100 to increase bank capacity ~ 1000 sp_*`);
         const deduct1 = await eco.deduct(user, "REVERSE KING", 100);
         const add1 = eco.giveCapacity(user,"REVERSE KING", 1000);
       ...
       if (k < balance.wallet) return message.reply(`*You need to pay 🪙1000 to increase bank capacity ~ 100000 sp*`);
         const deduct2 = await eco.deduct(user,"REVERSE KING", 1000);
         const add2 = eco.giveCapacity(user, "REVERSE KING", 100000);
       ...
       if (k < balance.wallet) return message.reply(`You need to pay 🪙10000 to increase bank capacity ~ 1000 sp`);
          const deduct3 = await eco.deduct(user, "REVERSE KING", 10000);
          const add3 = eco.giveCapacity(user, "REVERSE KING", 10000000);
